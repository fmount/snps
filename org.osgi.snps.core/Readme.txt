+ Remove "This Plugin is a singleton"
+ Rename "plugin.xml" => "plugin.xml.tobuild"
...
+ Goto Core Activator and Decomment The piece of code about "Debug Mode" (CommandLine)
+ Re-Configure launch configuration:
	+ Deselect all and then select only middleware bundles
	
+ In order to show online WSDL, on VM you can add this iptables rule:
...
+ In order to grant access to mysqldb, goto mysql and add this:
   + GRANT SELECT,INSERT,UPDATE,DELETE,CREATE,DROP ON SensorDB.* TO 'root'@'192.168.0.8' IDENTIFIED BY 'francesco';
   (you can customize dbname, username, host (ifconfig before), password)

+ add a config.xml on SNPSMDW folder containing:
		<client>
			<config>
				<ip>192.168.0.8</ip>
				<port>9090</port>
